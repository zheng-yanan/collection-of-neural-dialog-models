...
class Seq2Seq(BaseTFModel):
	...
	def __init__(self, sess, config, api, log_dir, forward, scope=None):
		...
		with variable_scope.variable_scope("generationNetwork"):
			...
			if config.num_layer > 1:
				...
				for i in range(config.num_layer):
					temp_init = layers.fully_connected(context_embedding, self.dec_cell_size, activation_fn=None,
				...
				dec_init_state = layers.fully_connected(context_embedding, self.dec_cell_size, activation_fn=None,
														scope="init_state")
				if config.cell_type == 'lstm':
		...
		with variable_scope.variable_scope("decoder"):
			...
			if forward:
				...
				loop_func = decoder_fn_lib.context_decoder_fn_train(dec_init_state, context_vector)
	...
	def attention(self, rnn_outputs, dimension, attention_dim):

		# [batch_size, seq_len, hidden_dim]
		inputs = rnn_outputs
		hidden_dim = dimension
		
		w_omega = tf.Variable(tf.random_normal([hidden_dim, attention_dim], stddev=0.1))
		b_omega = tf.Variable(tf.random_normal([attention_dim], stddev=0.1))
		u_omega = tf.Variable(tf.random_normal([attention_dim], stddev=0.1))
		...
		vu = tf.reshape(vu, [-1, self.max_dialog_len])
