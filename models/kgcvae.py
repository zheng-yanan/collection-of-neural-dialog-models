...
class KGCVAE(BaseTFModel):
	def __init__(self, sess, config, api, log_dir, forward, scope=None):
		...
		if not forward:
			with variable_scope.variable_scope("loss"):
				...
				aug_elbo = self.elbo # self.avg_bow_loss + self.avg_da_loss + self.elbo
	...
	def batch_2_feed(self, batch, global_t, use_prior, repeat=1):
		...
		feed_dict = {self.input_contexts: context, 
					 ...
					 self.floors: floors, 
					 self.topics:topics, 
					 #self.my_profile: my_profiles,
					 #self.ot_profile: ot_profiles, 
					 self.output_tokens: outputs,
					 self.output_das: output_das, 
